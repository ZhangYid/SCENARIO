...
def single_source_shortest_path_length(G, source,target, cutoff=None):
    ...
    if cutoff is None:
        cutoff = float('inf')
    nextlevel = {source: 1}
    return dict(_single_shortest_path_length(G.adj, nextlevel, cutoff,target))


def _single_shortest_path_length(adj, firstlevel, cutoff,nodes2):
...
def need_pairs_shortest_path_length(G,nodes1,nodes2, cutoff=None):

    length = single_source_shortest_path_length
    # TODO This can be trivially parallelized.
    for n in nodes1:
        yield (n, length(G, n, nodes2,cutoff=cutoff))
