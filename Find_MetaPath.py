...
class FMP_algo:
    ...
    def get_maxlength(self, gene1, gene2):
        before_vis1 = []
        before_vis2 = []
        new_vis1 = []
        new_vis2 = []
        before_vis1.append(gene1)
        before_vis2.append(gene2)
        k = 0
        was_connect = 0
        ...
        while (1):
            is_connect = 0
            k += 1
            for nod in before_vis1:
                current = nod.split("-")[-1]
                for neighbor in self.graph.neighbors(current):
                    if neighbor in current:
                        continue
                    if self.get_node_type(neighbor) == 'G':
                        continue
                    current_node = nod + "-" + neighbor
                    new_vis1.append(current_node)
            for nod in before_vis2:
                current = nod.split("-")[-1]
                for neighbor in self.graph.neighbors(current):
                    if neighbor in current:
                        continue;
                    if self.get_node_type(neighbor) == 'G':
                        continue
                    for test_nodes in new_vis1:
                        test_node1 = test_nodes.split("-")[-1]
                        test_node2 = test_nodes.split("-")[-2]
                        if test_node2 != current and test_node1 == neighbor:
                            is_connect = 1
                    current_node = nod + "-" + neighbor
                    new_vis2.append(current_node)
            if was_connect == 1 and is_connect == 0:
                return k
            if k==4:
                return k
            was_connect = is_connect
            before_vis1 = copy.deepcopy(new_vis1)
            before_vis2 = copy.deepcopy(new_vis2)
